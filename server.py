...
def generate_pdf(mcqs: List[MCQData], topic: str, job_id: str, relevant_mcqs: int, irrelevant_mcqs: int, total_links: int) -> str:
    ...
    try:
        ...
        if LAMBDA_S3_INTEGRATION and os.environ.get('ENVIRONMENT') == 'lambda':
            try:
                s3_url = upload_pdf_to_s3_lambda(str(filepath), job_id, topic, "SSC")
                if s3_url:
                    print(f"✅ PDF uploaded to S3: {s3_url}")
                    # Return S3 URL instead of local filename for Lambda
                    return s3_url
                else:
                    print("⚠️ S3 upload failed, falling back to local file")
            except Exception as e:
                print(f"⚠️ S3 upload error: {e}, using local file")
...
def generate_image_based_pdf(screenshots_data: List[dict], topic: str, exam_type: str = "SSC", job_id: str = None) -> str:
    ...
    try:
        ...
        if LAMBDA_S3_INTEGRATION and os.environ.get('ENVIRONMENT') == 'lambda':
            try:
                s3_url = upload_pdf_to_s3_lambda(str(filepath), job_id, topic, exam_type)
                if s3_url:
                    print(f"✅ Image PDF uploaded to S3: {s3_url}")
                    # Return S3 URL instead of local filename for Lambda
                    return s3_url
                else:
                    print("⚠️ S3 upload failed, falling back to local file")
...
async def process_screenshot_extraction_ultra_robust(job_id: str, topic: str, exam_type: str, links: List[str]):
    ...
    try:
        ...
        try:
            ...
            filename = generate_image_based_pdf(screenshot_data, topic, exam_type, job_id)
            pdf_url = f"/api/download-pdf/{filename}"
